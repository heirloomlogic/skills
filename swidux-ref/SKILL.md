...
| Keep a paid user from being shown as free on a transient/slow/throwing entitlement read (last-known-good gate) | Wrap the `PaywallService` in `ResilientPaywallService(base:store:)` and pass the same instance to both `PaywallPlugin(service:)` and any other entitlement reader (see `references/swidux-paywall.md`) |
...
- ❌ Hardening only an app-level payload/context closure for entitlement resilience. That protects a server payload, not `PaywallState` — the slice that actually decides whether a user is shown as free. Wrap the `PaywallService` in `ResilientPaywallService` and feed it to `PaywallPlugin(service:)` instead (and to any other entitlement reader) — see `references/swidux-paywall.md` → "Last-known-good entitlement resilience"
...
- `SwiduxPaywall` — paywall + entitlement plugin (RevenueCat or StoreKit-shaped); also ships the SDK-free `SimulatedPaywallService` dev default and `ResilientPaywallService`, a last-known-good decorator that wraps any `PaywallService` so a transient entitlement-read failure never presents a paid user as free
